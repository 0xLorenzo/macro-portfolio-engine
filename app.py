...
# 尝试导入 openai，不存在或未配置 Key 使用默认概率
try:
    import openai
...
st.set_page_config(page_title="宏观投资引擎 V5.3 安全版", layout="wide")
...
@st.cache_data
def load():
    data = yf.download(ASSETS, period="3y")["Close"]  # 至少 3 年数据
    return data.dropna()
...
def gpt_macro(features):
    if not GPT_AVAILABLE:
        return {
            ...
            "reasoning": "未配置 OpenAI Key，使用默认概率", 
            "risk": "无法判断"
        }
    try:
        prompt = f"""
...
{"probabilities":{"复苏":0,"过热":0,"衰退":0,"滞胀":0},"reasoning":"","risk":""}
"""
